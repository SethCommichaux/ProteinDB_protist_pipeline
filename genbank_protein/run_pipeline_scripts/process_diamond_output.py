...
def build_taxa_dicts():
	fullnamelineage = {}
	ids = {i.strip().split('\t')[1]:0 for i in open(args.d)}
	for h,i in enumerate(open(args.t)):
		...
		if gb_id in ids:
			...
			ko = tmp[7]
			eggnog = tmp[8]
			fullnamelineage[gb_id] = [prot_len,lineage,prot_name,kegg,ko,eggnog]
...
def LCA(taxa):
	...
	for i,j in enumerate(taxa):
		if 'EUKARYOTA' not in j:
			...
			c = 0
			j = map(lambda z: z.strip(), j.split(';'))
			if i == 0:
				new_lineage = j
			else:
...
def calc_ANI(pident,aln):
	pident = map(lambda x:x/100.0,pident)
	total_len = sum(aln)
	weight = 0
	for i in range(len(pident)):
		weight += pident[i]*aln[i]
	ANI = weight/float(total_len)
	if ANI < args.ANI/100.0: return False
...
with open(args.d+'.results','w') as out, open(args.d+'.functions','w') as out2:
	...
	out2.write('Phylogroup\tProteinID\tProteinName\tKEGG\tKO\teggNOG\n')
	for k,v in sorted(cumulative_results.items()):
		if v[0] < int(args.num_reads): continue
		if len(v[1]) < int(args.num_prots): continue
		if 'EUKARYOTA' in k:
			if 'METAZOA' not in k:
				if 'FUNGI;' not in k:
					if 'EMBRYOPHYTA' not in k:
						...
						for i in set(v[1]):
							if i in fullnamelineage:
								out2.write(str(k)+'\t'+str(i)+'\t'+fullnamelineage[i][4]+'\t'+fullnamelineage[i][6]+'\t'+fullnamelineage[i][7]+'\t'+fullnamelineage[i][8]+'\n')
